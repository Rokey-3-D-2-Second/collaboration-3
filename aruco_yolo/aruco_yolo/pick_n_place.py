...
class ArucoMarkerListener(Node):
    def __init__(self):
        ...
        self.state_publisher = self.create_publisher(String, 'change_state', 10)
...
##################################################################################
    ...
    def aruco_arm_controll(self, msg):

        print("Impossible Mission Start")        
        arm_client = TurtlebotArmClient()

        print(f"Mission Aruco marker Location coordinates: {self.aruco_location_x}, {self.aruco_location_y}, {self.aruco_location_z}")
        ...
        if self.aruco_marker_found:
            ...
            response = arm_client.send_request(1, "lane_tracking_02")
            ...
            time.sleep(1)
            print("Move to Place...")
            response = arm_client.send_request(1, "place")
            arm_client.get_logger().info(f'Response: {response.response}')
...
def main(args=None):
    ...
    # point.positions = [0.087, -0.28, -0.70, 2.04] 

    # trajectory_msg.points = [point]
    # joint_pub.publish(trajectory_msg)

    TurtlebotArmClient().send_request(1, "lane_tracking_02")
